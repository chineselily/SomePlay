...
Shader "Nature/Afs Tree Creator Bark Deferred Optimized" {
Properties {
...
SubShader { 
	...
	#include "Includes/AfsBlinnPhongLightingFull.cginc"
	...
	//	Add extra animation to make it fit speedtree
		...
		fOsc = (fOsc + 3.0) * 0.33;
	//	Apply Wind
		v.vertex = afsAnimateVertex( v.vertex, float4(v.normal.xyz,fOsc), float4(v.color.xy, v.texcoord1.xy), float4(0,0,0,_SquashAmount) );
